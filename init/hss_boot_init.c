...
static bool getBootImageFromSpiFlash_(struct HSS_BootImage **ppBootImage) {
    ...
    static uint8_t first800_buf[0x800];
    memset(first800_buf, 0, 0x800);
    result = SpiFlashReadBlock(first800_buf, 0, 0x800);
    if (!result)
        return false;
    mb();
    for (int i = 0; i < 0x800 / 0x40; i++) {
        mHSS_PRINTF("%x ||   ", i * 0x40);
        for (int j = 0; j < 0x40; j++) {
            mHSS_PRINTF("%02x ", first800_buf[i * 0x40 + j]);
        }
        mHSS_PUTC('\n');
    }
